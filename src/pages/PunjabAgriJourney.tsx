...
import {
  PB_AGRI_ALL, PB_DEMANDS, filterRows, computeKpis, buildHierarchy,
  getMajorHeads, topMovers, getDemandSummaries, formatCr,
  type DemandFilter, type HierNode, type PBAgriRow,
...
function MajorHeadsTable({ rows }: { rows: PBAgriRow[] }) {
  ...
  return (
    <div className="flex flex-col">
      <div className="relative mb-3 max-w-md">
        ...
        <Input placeholder="Search heads…" value={q} onChange={(e) => setQ(e.target.value)} className="pl-8 h-9" />
      </div>
...
function Movers({ rows }: { rows: PBAgriRow[] }) {
...
export default function PunjabAgriJourney() {
      ...
      <main className="flex-1">
        <section className="border-b border-border">
          <div className="container py-6">
            <Link to="/states/punjab" className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground">
              <ArrowLeft className="h-3.5 w-3.5" /> All Punjab departments
            </Link>
          </div>
        </section>
        ...
        <section className="border-b border-border">
          <div className="container py-10 md:py-14 text-left">
            <div className="text-xs uppercase tracking-[0.16em] text-primary font-medium">Punjab · Agriculture &amp; Horticulture · 2026-27</div>
            <h1 className="mt-3 font-serif text-3xl md:text-5xl font-semibold leading-tight">
              ਖੇਤੀਬਾੜੀ — two demands, one farm economy.
            </h1>
            <p className="mt-4 max-w-3xl font-serif text-base md:text-xl leading-relaxed text-foreground/85">
              Punjab's farm envelope is split across two Demands for Grants — Demand 1 (Agriculture) and
              Demand 34 (Horticulture). Every figure below traces back to the state Demands-for-Grants
              PDFs and is shown in ₹ Crore.
            </p>
          </div>
        </section>
        ...
        <section className="border-b border-border">
          <div className="container py-8 text-left">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-px bg-border border border-border">
              <Kpi label="BE 2026-27" value={formatCr(k.totalLakh)} sub={`${PB_DEMANDS.length} demands · ${PB_AGRI_ALL.length.toLocaleString()} rows`} />
        ...
        <section className="border-b border-border">
          <div className="container py-10 text-left">
            ...
            <h2 className="mt-2 font-serif text-2xl md:text-3xl font-semibold">Two demands, side-by-side</h2>
            <p className="mt-2 max-w-3xl text-sm text-muted-foreground">Click any card to filter the explorer below.</p>
            <div className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {PB_DEMANDS.map((d) => (
                <button key={d.id} onClick={() => setFilter(d.id)}
                  ...
                  D{String(d.no).padStart(3, "0")} · {d.title}
                </button>
              ))}
            </div>
        ...
        <section className="border-b border-border">
          <div className="container py-10 text-left space-y-4">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-muted-foreground">Click any row to expand.</p>
            </div>
        ...
        <section>
          <div className="container py-8 text-left text-[11px] text-muted-foreground">
            Source · Punjab Demands for Grants 2026-27, Demands 1 (Agriculture) and 34 (Horticulture).
            All figures in ₹ Crore (1 Cr = 100 Lakh). Original PDFs are referenced in the row-level source columns of the raw dataset.
          </div>
        </section>
      </main>
      <SiteFooter />
