// UP Agriculture selectors. Source values are in ₹ Lakh; we convert to ₹ Crore (÷100) for display.
import raw from "@/data/states/uttar-pradesh/agriculture/ddgs.json";

export type Section = "Revenue" | "Capital";
export type GapFlag = "NEW" | "DISCONTINUED" | "SMALL_BASE";

export interface UPAgriRow {
  id: string;
  demandId: string; // d010..d018
  demandNo: number;
  demandTitle: string;
  section: Section;
  majorHead: number;
...
export const UP_AGRI_ALL: UPAgriRow[] = raw as UPAgriRow[];

export const UP_DEMANDS = Array.from(
  new Map(UP_AGRI_ALL.map((r) => [r.demandId, { id: r.demandId, no: r.demandNo, title: r.demandTitle }])).values()
).sort((a, b) => a.no - b.no);

export type DemandFilter = "all" | string;

export function filterRows(f: DemandFilter): UPAgriRow[] {
  return f === "all" ? UP_AGRI_ALL : UP_AGRI_ALL.filter((r) => r.demandId === f);
}

// Lakh → Crore
export const toCr = (v: number | null | undefined) => ((v ?? 0) / 100);

export function sum(rows: UPAgriRow[], k: "actuals2425" | "be2526" | "re2526" | "be2627") {
...
export function computeKpis(rows: UPAgriRow[]): Kpis {
...
export interface HierNode {
  ...
  nameHi?: string;
  value: number;       // ₹ Lakh
  prevValue: number;   // ₹ Lakh
  level: "major" | "minor" | "sub" | "object";
  children?: HierNode[];
  rows?: UPAgriRow[];
}

export function buildHierarchy(rows: UPAgriRow[]): HierNode {
  const root: HierNode = { key: "root", name: "UP Agriculture", value: 0, prevValue: 0, level: "major", children: [] };
  ...
  for (const r of rows) {
    ...
    if (!major) {
      major = { key: majorKey, name: `${r.majorHead} · ${r.majorHeadName}`, nameHi: r.majorHeadNameHi, value: 0, prevValue: 0, level: "major", children: [] };
    ...
    if (!minor) {
      minor = { key: minorKey, name: r.minorHeadName || `Minor ${r.minorHead}`, nameHi: r.minorHeadNameHi, value: 0, prevValue: 0, level: "minor", children: [] };
    ...
    if (!sub) {
      sub = { key: subKey, name: r.subHeadName || `Sub-head ${r.subHead}`, value: 0, prevValue: 0, level: "sub", children: [], rows: [] };
...
export function getMajorHeads(rows: UPAgriRow[]): MajorHeadRow[] {
...
export function topMovers(rows: UPAgriRow[], minBaseLakh = 1000): { hikes: MoverRow[]; cuts: MoverRow[] } {
...
export function getDemandSummaries(): DemandSummary[] {
  return UP_DEMANDS.map((d) => {
    const rs = UP_AGRI_ALL.filter((r) => r.demandId === d.id);
